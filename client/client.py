...
def new_game():
    ...
    response = requests.post(api_url + '/games',headers=headers)
    return response.json()['id']

def get_board(id):
    headers = {'content-type': 'application/json'}
    response = requests.get(api_url + '/games/' + str(id),headers=headers)
    return response.json()['board']

def delete_game(id):
    headers = {'content-type': 'application/json'}
    response = requests.delete(api_url + '/games/' +str(id),headers=headers)
    return response.json()['id']

def put_stone(id, x, y, player, auto):
    headers = {'content-type': 'application/json'}
    body = {'x': x, 'y': y, 'player': player, 'auto': auto}
    response = requests.put(api_url + '/games/' + str(id),json.dumps(body), headers=headers)
